...
export class TradeRepositoryDatabase implements TradeRepository {
    ...
    async saveTrade(trade: Trade): Promise<void> {
        await this.connection.query("insert into platform_trading_db.trade (trade_id, market_id, buy_order_id, sell_order_id, side, quantity, price, timestamp) values ($1, $2, $3, $4, $5, $6, $7, $8)", [trade.tradeId, trade.marketId, trade.buyOrderId, trade.sellOrderId, trade.side, trade.quantity, trade.price, trade.timestamp]);
    }

    async getTradesByMarketId(marketId: string): Promise<Trade[]> {
        const tradesData = await this.connection.query("select * from platform_trading_db.trade where market_id = $1", marketId);
        const trades: Trade[] = [];
