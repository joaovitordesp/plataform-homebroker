...
export class RabbitMQAdapter implements Queue {
    ...
    async publish(event: string, input: any): Promise<void> {
        await this.channel.assertExchange(event, "direct", { durable: true });
        await this.channel.assertQueue(event, { durable: true });
        this.channel.bindQueue(event, event, "");

        await this.channel.publish(event, "", Buffer.from(JSON.stringify(input)));
    }
